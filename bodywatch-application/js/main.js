...
function showNotificaton() {
  ...
  if (muteCheck == 'muted' && showPopup == 'true') {
    ...
    console.log(
      'sound is muted and showPopup is false so nothing shows but it went of in the background!'
    );
    resetLeanCheck();
...
function randomNotifications() {
  ...
  setTimeout(function () {
    Push.create('Time for a break!', {
      body:
        'Click on this notification to take a break. ' +
        frequentTime * 2 +
        ' minutes have passed.',
...
function testing_load() {
  ...
  document.getElementById('myPopup').innerHTML = localStorage.getItem(
    'notification'
  );
  document.getElementById('myFrequentTime').value = localStorage.getItem(
    'frequenttimer'
  );
  document.getElementById('myGoodPoseTime').value = localStorage.getItem(
    'goodposetimer'
  );
  document.getElementById('start').innerHTML = localStorage.getItem(
    'startpositie'
  );
...
function changeColorToGood() {
  document.getElementById('gwd-div-uhf8').style.backgroundColor =
    'rgb(' + 63 + ',' + 255 + ',' + 0 + ')';
  document.getElementById('gwd-span-1rvu').innerHTML =
    'Your posture is correct! Good job :)';
}
...
function showStatistics() {
  ...
  document.getElementById('timeWorked').innerHTML =
    lS_diffHrs + 'h : ' + lS_diffMins + 'm';
}
...
function calculateSpendTime() {
  ...
  console.log(
    diffDays +
      ...
      ' hours, ' +
      diffMins +
      ' minutes spend using this application'
  );
}
...
function randomTips() {
  ...
  setTimeout(function () {
    textField.innerHTML =
      'Please take short brakes, dont work for more than two hours at the time';
  }, tipsTime * 60 * 1000);
  ...
  setTimeout(function () {
    textField.innerHTML =
      'This software is not a medical expert, please see one if needed';
  }, tipsTime * 3 * 60 * 1000);

  setTimeout(function () {
    textField.innerHTML =
      'Stress can cause backpains! Remember to take a break once in a while!';
  }, tipsTime * 4 * 60 * 1000);

  setTimeout(function () {
    textField.innerHTML =
      'Stuck on something, try to take a little break to clear your mind.';
  }, tipsTime * 5 * 60 * 1000);

  setTimeout(function () {
    textField.innerHTML =
      'Did you know that a good pose, also improves your productivity.';
  }, tipsTime * 6 * 60 * 1000);
